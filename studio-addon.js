/* DreamWall publishing + wallpaper display fix.
   This file intentionally sits after index.html so it can safely override
   the old publishing/viewer code without changing the main app. */
(async()=>{
  ...
  async function loadWallpapersFixed(){
    ...
    if(ids.length){
      ...
      if(p.error)console.error("DreamWall profile load error:",p.error);
      else profiles=p.data||[];
    }
  ...
  async function loadLikesFixed(){
    ...
    (r.data||[]).forEach(x=>{
      ...
      if(window.user&&String(x.user_id)===String(window.user.id))mine[x.wallpaper_id]=true;
    });
  ...
  function showWallpaper(w){
    viewer();
    renderWallpaperInto($("dwViewerWall"),w);
    $("dwWallpaperViewer").style.display="grid";
  }
  ...
  function renderWallElements(){
    ...
    document.querySelectorAll("[data-dw-wallpaper-id]").forEach(el=>{
      ...
      if(w){
        renderWallpaperInto(el,w);
        el.onclick=()=>showWallpaper(w);
      }
    });
  }
  ...
  function buildHome(){
    ...
    feed.innerHTML=(wallpapers||[]).map(w=>{
      const p=w.profiles||{};
      const name=p.real_name||p.username||"DreamWall creator";
      const handle=p.username?"@"+p.username:"@creator";
      return `<article class="post">
        <div class="posthead" style="cursor:default">
          <div class="avatar">${p.avatar_url?`<img src="${esc(p.avatar_url)}">`:esc(name.charAt(0).toUpperCase())}</div>
          <div><div class="postname">${esc(name)}</div><div class="handle">${esc(handle)}</div><div class="post-time">${typeof timeAgo==="function"?timeAgo(new Date(w.created_at)):""}</div></div>
        </div>
        <div class="wall" data-dw-wallpaper-id="${esc(w.id)}"></div>
        <div class="actions">
          <button class="iconbtn" onclick="window.dwLike('${esc(w.id)}')">♡</button>
          <button class="iconbtn" onclick="window.dwOpen('${esc(w.id)}')">🔍</button>
          <button class="iconbtn save">🔖</button>
        </div>
        <div class="likes">${Number(w.likes_count||0)} likes</div>
        <div class="caption">${esc(w.caption||"")}</div>
      </article>`;
  ...
  function buildGrid(id,onlyMine=false){
    const box=$(id);
    if(!box)return;
    const rows=(wallpapers||[]).filter(w=>!onlyMine||String(w.user_id)===String(window.user?.id));
  ...
  /* Instagram-like 24-hour wallpaper viewer for a DreamWaller name/avatar. */
  function installPeopleViewer(){
    if($("dwPeopleViewer"))return;
    const modal=document.createElement("div");
    modal.id="dwPeopleViewer";
    modal.style.cssText="position:fixed;inset:0;background:rgba(0,0,0,.86);display:none;place-items:center;z-index:9998;padding:20px";
    ...
    document.body.appendChild(modal);
    modal.addEventListener("click",e=>{if(e.target===modal)modal.style.display="none"});
    $("dwPeopleClose").onclick=()=>modal.style.display="none";
  }
  ...
  function openRecentWallpapers(id){
    ...
    $("dwPeopleTitle").textContent=p.real_name||p.username||"DreamWaller";
    $("dwPeopleSub").textContent=`@${p.username||"username"} • ${recent.length} wallpaper${recent.length===1?"":"s"} in the last 24 hours`;
    ...
    renderWallElements();
    $("dwPeopleViewer").style.display="grid";
  }
  ...
  window.dwOpen=id=>{
    const w=(wallpapers||[]).find(x=>String(x.id)===String(id));
    if(w)showWallpaper(w);
  };

  window.dwLike=async id=>{
    if(!window.user){$("authModal")?.classList.add("open");return;}
    const existing=await api.from("likes").select("wallpaper_id").eq("wallpaper_id",id).eq("user_id",window.user.id).maybeSingle();
    if(existing.data){
      await api.from("likes").delete().eq("wallpaper_id",id).eq("user_id",window.user.id);
    }else{
      const r=await api.from("likes").insert({wallpaper_id:id,user_id:window.user.id});
      if(r.error&&r.error.code!=="23505")console.error(r.error);
    }
    await loadLikesFixed();
    buildHome();
  ...
  async function refresh(){
    ...
    const ok=await loadWallpapersFixed();
    if(!ok)return;
    await loadLikesFixed();
    renderPeopleFixed();
    buildHome();
  ...
  if(publish)publish.onclick=async()=>{
    ...
    try{
      ...
      /* IMPORTANT: do not select the profiles relation on INSERT.
         That relation can be ambiguous in Supabase and is what caused
         the old "more than one relationship" error. */
      const result=await api.from("wallpapers").insert({
      ...
      await refresh();
      if(!(wallpapers||[]).some(w=>String(w.id)===String(result.data.id))){
        throw new Error("Wallpaper was saved, but it could not be loaded back into DreamWall.");
      }
      $("postCaption").value="";
  ...
  /* The original app loads auth asynchronously. Keep refreshing after auth,
     rather than relying on a fixed 700/2200ms timeout. */
  const originalLoadApp=window.loadApp;
  if(typeof originalLoadApp==="function"){
    window.loadApp=async function(){
      const result=await originalLoadApp.apply(this,arguments);
  ...
  if(typeof sb!=="undefined"){
    sb.auth.onAuthStateChange(()=>setTimeout(refresh,100));
  }
